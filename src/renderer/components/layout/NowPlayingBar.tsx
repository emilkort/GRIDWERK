...
export default function NowPlayingBar() {
  ...
  // Find the currently playing sample from the store
  ...
  // Measure waveform container width for responsive canvas
  ...
  // Don't render if nothing has been played
  if (!currentFilePath) return null
    ...
    <div className="h-16 border-t border-border bg-elevated flex items-center gap-3 px-4 shrink-0">
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          className="flex items-center justify-center w-9 h-9 rounded-full bg-white hover:bg-accent text-black hover:text-white transition-all duration-200"
        >
          {isPlaying ? (
      ...
      {/* Time - current */}
      <span className="text-text-muted text-[11px] font-mono min-w-[32px] text-right">
        {formatTime(currentTime)}
      </span>

      {/* Waveform / progress */}
      <div ref={waveContainerRef} className="flex-1 h-10 flex items-center">
      ...
      {/* Time - duration */}
      <span className="text-text-muted text-[11px] font-mono min-w-[32px]">
        {formatTime(duration)}
      </span>
    </div>
  )
